...
A language-agnostic baseline for AI-assisted development, deployable to Claude Code and
OpenCode with `bun-off`.
...
- `llm-tldr`: compressed code-intelligence context for exploring a codebase. A
  `post_install` hook runs `llm-tldr warm .` to build its semantic index for the project
  (skipped with a hint if the binary is absent).
...
The bundle declares its own tools (rtk and llm-tldr) in `mise/mise.toml` and installs them on
deploy: a `mise-install` `post_install` hook runs `mise trust` + `mise install`, and a
`session_start` event hook refreshes them each session (both no-ops once the tools are
present). These drop-ins land under `.config/mise/conf.d/` and are dedicated to the bundle's
tooling: they are independent of any `mise.toml` the edited project uses for its own runtime. If mise itself is not installed, the hook skips gracefully and the tool hooks print a
one-time install hint instead of failing the deploy.
...
these rules and servers automatically.
